...
{
    Task RestoreDatabase(string backupFilePath, BackupType type = BackupType.Full, CancellationToken cancellationToken = default);
    ...
    async Task RestoreChain(IReadOnlyList<(string FilePath, BackupType Type)> chain, CancellationToken cancellationToken = default)
    {
        foreach (var step in chain)
            await RestoreDatabase(step.FilePath, step.Type, cancellationToken);
    }
}
